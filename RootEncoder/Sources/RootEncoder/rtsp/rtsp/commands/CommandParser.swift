...
class CommandParser {
    ...
    func loadServerPorts(command: RtspCommand, protocol: Protocol, audioClientPorts: [Int], videoClientPorts: [Int], audioServerPorts: inout [Int], videoServerPorts: inout [Int]) -> Bool {
        var isAudio = true
    ...
    func getSessionId(command: RtspCommand) -> String {
    ...
    func parseResponse(method: Method, responseText: String) -> RtspCommand {
        let status = getResponseStatus(response: responseText)
        let cSeq = getCSeq(request: responseText)
        return RtspCommand(method: method, cSeq: cSeq, status: status, text: responseText)
    }
        
    func parseCommand(commandText: String) -> RtspCommand {
        let method = getMethod(response: commandText)
        let cSeq = getCSeq(request: commandText)
        return RtspCommand(method: method, cSeq: cSeq, status: -1, text: commandText)
    }
